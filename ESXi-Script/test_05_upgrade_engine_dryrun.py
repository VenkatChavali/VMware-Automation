...
def test_full_dry_run():
    ...
    print(f"Storage health    : {'OK OK' if result.storage_health_ok else 'FAIL FAIL'}")
    print(f"Overall           : {'OK OK' if result.overall_ok        else 'FAIL FAIL'}")
...
def test_phase0_only():
    ...
    try:
        ...
        print(f"FAIL Phase 0 failed: {exc}")
        import traceback
        traceback.print_exc()
    finally:
        if engine._vc:
