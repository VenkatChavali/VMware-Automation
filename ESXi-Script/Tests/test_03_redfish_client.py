...
def test_01_connectivity():
    ...
    with RedfishClient(IDRAC_IP, IDRAC_USER, IDRAC_PASS, logger) as rf:
        ...
        if reachable:
            print("✓ iDRAC is responding to Redfish API calls")
        else:
            print("✗ Cannot reach iDRAC — check IP, credentials, and network")
...
def test_05_upload_single_firmware():
    ...
    with RedfishClient(IDRAC_IP, IDRAC_USER, IDRAC_PASS, logger) as rf:
        ...
        if job_id and job_id != "DRY-RUN-JOB":
            ...
            print(f"Task result: {'✓ Scheduled/Complete' if success else '✗ Failed'}")
...
def test_06_dry_run_upload():
    ...
    with RedfishClient(IDRAC_IP, IDRAC_USER, IDRAC_PASS, logger) as rf:
        ...
        print("✓ Dry-run completed — no changes made to iDRAC")
