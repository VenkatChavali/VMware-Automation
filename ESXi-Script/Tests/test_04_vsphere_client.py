...
def test_01_vcenter_connection():
    ...
    try:
        ...
        print(f"✓ Found host object   : {vc.esxi_host}")
        vc.disconnect()
        return True
...
def test_03_get_idrac_ip():
    ...
    try:
        ...
        if idrac_ip:
            print(f"✓ iDRAC IP found: {idrac_ip}")
        else:
            print("✗ iDRAC IP not found via vSphere API")
...
def test_04_nic_snapshot():
    ...
    try:
        ...
        print(f"\nSelf-comparison health check: {health_ok}  ✓ (should be True)")
...
def test_07_ssh_connectivity():
    ...
    try:
        ...
        print("SSH service enabled on host ✓")
        ...
        if rc == 0:
            print("\n✓ SSH esxcli command successful")
        else:
            print("\n✗ SSH command failed — check root credentials and SSH access")
...
def test_09_alarm_toggle():
    ...
    try:
        ...
        print("✓ Alarm actions re-enabled")
...
def test_10_esxi_upgrade_dry_run():
    ...
    try:
        ...
        print(f"\nDry-run result: {'✓ Would succeed' if result else '✗ Would fail'}")
        print("No changes were made to the host ✓")
...
def test_11_maintenance_mode():
    ...
    try:
        ...
        print(f"Enter MM result: {'✓ Success' if ok else '✗ Failed'}")
