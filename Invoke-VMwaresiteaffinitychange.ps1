function Invoke-VMwareSiteAffinityMigration {
    ...
    if ($vcList.Count -eq 0) {
        try {
            $vcList = @($Global:VMwareSessions.Keys | Where-Object { $Global:VMwareSessions[$_].IsConnected })
        } catch {}
    }
    ...
    if (-not $plan) {
        Write-Warning "[$funcName] Migration plan could not be created."
        return
    }
    ...
    try { if ($plan.ContainsKey('Skipped'))      { $planSkipped      = @($plan['Skipped']) } } catch {}
    try { if ($plan.ContainsKey('Batches'))      { $planBatches      = @($plan['Batches']) } } catch {}
    try { if ($plan.ContainsKey('TotalVMs'))     { $planTotalVMs     = [int]$plan['TotalVMs'] } } catch {}
    try { if ($plan.ContainsKey('TotalBatches')) { $planTotalBatches = [int]$plan['TotalBatches'] } } catch {}
    try { if ($plan.ContainsKey('VmObjMap'))     { $planVmObjMap     = $plan['VmObjMap'] } } catch {}
    try { if ($plan.ContainsKey('HostObjMap'))   { $planHostObjMap   = $plan['HostObjMap'] } } catch {}

    if ($planSkipped.Count -gt 0) {
        Write-Host "`n[$funcName] ----- Skipped VMs ($($planSkipped.Count)) -----" -ForegroundColor Yellow
        $planSkipped | Select-Object VMName,VCenter,Cluster,Status,Remarks | Format-Table -AutoSize
    }

    if ($planTotalVMs -eq 0) {
    ...
    if (-not $PSCmdlet.ShouldProcess(
        "$planTotalVMs VM(s) across $planTotalBatches batch(es)",
    ...
    Write-Host "  Total VMs  : $planTotalVMs"
    Write-Host "  Batches    : $planTotalBatches"
    ...
    function Select-TargetHost {
        ...
        for ($a = 0; $a -lt $hids.Count; $a++) {
            ...
            $hid = $hids[$idx]
            $h   = $null
            if ($planHostObjMap.ContainsKey($hid)) { $h = $planHostObjMap[$hid] }
            if (-not $h) { continue }
    ...
    function Dispatch-VMTask {
        ...
        $vmObj = $null
        if ($planVmObjMap.ContainsKey($task.VmMoRefId)) {
            $vmObj = $planVmObjMap[$task.VmMoRefId]
        } elseif ($planVmObjMap.ContainsKey($task.VmMoRefVal)) {
            $vmObj = $planVmObjMap[$task.VmMoRefVal]
        }
    ...
    function Poll-RunningTasks {
        ...
        foreach ($t in $toScan) {
            $pt  = $null
            if ($taskIndex.ContainsKey($t.TaskId)) { $pt = $taskIndex[$t.TaskId] }
            ...
            if ($pt -and $pt.State -in @("Success","Error")) {
                ...
                if (-not $isSuccess) {
                    try {
                        if ($pt.ExtensionData -and $pt.ExtensionData.Error) {
                            $errMsg = $pt.ExtensionData.Error.LocalizedMessage
                        }
                    } catch {}
                }
            } elseif (-not $pt) {
                ...
                try {
                    ...
                    if ($viSrv -and $viSrv.IsConnected) {
                        ...
                        if ($vmNow) {
                            ...
                            if (-not $isSuccess) {
                                $errMsg = "Task purged — VM on $($vmNow.VMHost.Name) (not in target site)"
                            }
                        } else {
                            $isSuccess = $false
                            $errMsg    = "Task purged — VM not found"
                        }
                    } else {
                        $isSuccess = $false
                        $errMsg    = "Task purged — vCenter not connected"
                    }
                } catch {
                    $isSuccess = $false
                    $errMsg    = "Task purged — verify failed: $($_.Exception.Message)"
                }
        ...
        foreach ($vc in $VCenters) {
            ...
            try {
                ...
                foreach ($h in $hostObjs) {
                    ...
                    if ($planHostObjMap.ContainsKey($hid)) {
                        $cpu = $null
                        $mem = $null
                        try {
                        ...
                        $planHostObjMap[$hid].CpuPct = $cpu
                        $planHostObjMap[$hid].MemPct = $mem
                    }
                }
            } catch {}
        }
    ...
    foreach ($batch in $planBatches) {
        ...
        foreach ($ck in $clustersByVc.Keys) {
            ...
            try {
                ...
                foreach ($t in $cTasks) {
                    $vmObj = $null
                    if ($planVmObjMap.ContainsKey($t.VmMoRefId)) {
                        $vmObj = $planVmObjMap[$t.VmMoRefId]
                    } elseif ($planVmObjMap.ContainsKey($t.VmMoRefVal)) {
                        $vmObj = $planVmObjMap[$t.VmMoRefVal]
                    } else {
                        foreach ($k in $planVmObjMap.Keys) {
                            if ($k -like "*$($t.VmMoRefVal)*") {
                                $vmObj = $planVmObjMap[$k]
                                break
                            }
                        }
                    }
                    ...
                    if (-not $vmObj) {
                        ...
                        $t.AffinityStatus = "FAILED"
                        $t.AffinityRemark = "VM object not found in plan map"
                        continue
                    }

                    if ($t.SourceSite -eq "site01") {
                        [void]$toSite02.Add($vmObj)
                    } else {
                        [void]$toSite01.Add($vmObj)
                    }
                }
                ...
                foreach ($t in $cTasks) {
                    $t.AffinityStatus = "FAILED"
                    $t.AffinityRemark = "DRS error: $errMsg"
                    $t.Status         = "FAILED"
                    $t.Remarks        = "DRS affinity change failed"
                }
        ...
        foreach ($t in @($tasks | Where-Object { $_.AffinityStatus -eq "SUCCESS" -and $_.Status -eq "QUEUED" })) {
            ...
            while ($true) {
                ...
                if ($vr -lt $MaxConcurrentPerVC -and $cr -lt $MaxConcurrentPerCluster -and $totalRunning -lt $MaxGlobalConcurrent) {
                    break
                }

                Start-Sleep -Seconds 3
                $waited += 3
                if ($waited -gt ($MaxMonitorMinutes * 60)) {
                    $t.Status  = "TIMEOUT"
                    $t.Remarks = "Timed out waiting for slot"
                    break
                }

                $prevCount = $runningList.Count
                Poll-RunningTasks -Tasks $tasks -VcRunning $vcRunning -ClRunning $clRunning -TotalRunning $trRef -RunningList $runningList
        ...
        foreach ($t in @($tasks | Where-Object { $_.Status -eq "RUNNING" })) {
            $t.Status  = "TIMEOUT"
            $t.Remarks = "Timed out after $MaxMonitorMinutes min"
        ...
        if ($toRetry.Count -gt 0) {
            ...
            foreach ($t in $toRetry) {
                ...
                while ($true) {
                    ...
                    if ($waited -gt 300) {
                        $t.Status  = "FAILED"
                        $t.Remarks = "Retry slot timeout"
                        break
                    }

                    Poll-RunningTasks -Tasks $toRetry -VcRunning $vcRunning -ClRunning $clRunning -TotalRunning $trRef -RunningList $null
                    $totalRunning = $trRef.Value
                }
            ...
            foreach ($t in @($toRetry | Where-Object { $_.Status -eq "RUNNING" })) {
                $t.Status  = "FAILED"
                $t.Remarks = "Retry timed out"
            }
        }

        foreach ($t in @($tasks | Where-Object { $_.Status -eq "FAILED_PENDING_RETRY" })) {
            $t.Status  = "FAILED"
            $t.Remarks = "Max retries ($MaxRetry) exhausted"
        }

        # ---- Step E: Post-batch verification ----
        ...
        foreach ($t in @($tasks | Where-Object { $_.Status -eq "SUCCESS" })) {
            ...
            try {
                $vmId = if ($planVmObjMap.ContainsKey($t.VmMoRefId)) { $t.VmMoRefId } else { $t.VmMoRefVal }
                ...
                $t.VerifyRemark = $_.Exception.Message
            }
        }
